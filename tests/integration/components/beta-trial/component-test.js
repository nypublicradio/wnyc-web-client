...
module('Integration | Component | beta trial', function(hooks) {
  ...
  test('it renders into wormholes', async function(assert) {
    let beta = generateBetaTrial();
    beta.set('preBeta', true);
    this.set('beta', beta);

    await render(hbs`<div id="foo"></div>`);
    await render(hbs`<div id="bar"></div>`);
    await render(hbs`{{beta-trial beta=beta betaInvite='bar' legacyNavLink='foo'}}`);
  ...
  test('if this is a preBeta app, it adds landing sites to the alien DOM', async function(assert) {
    let beta = generateBetaTrial();
    beta.set('preBeta', true);
    this.set('beta', beta);
  ...
  test('beta invite can be dismissed', async function(assert) {
    assert.expect(4);
    ...
    await render(hbs`{{beta-trial beta=beta onDismiss=(action externalAction)}}`);
  ...
  test('beta invite can be entered', async function(assert) {
    assert.expect(2);
    ...
    await render(hbs`{{beta-trial beta=beta enterBeta=(action externalAction)}}`);
  ...
  test('if this is not a preBeta app, it will not add landing sites to the alien DOM', async function(assert) {
    let beta = generateBetaTrial();
    this.set('beta', beta);
  ...
  test('it can render an about dialog', async function(assert) {
    let beta = generateBetaTrial();
    beta.isBetaSite = true;

    this.set('beta', beta);
    await render(hbs`{{beta-trial beta=beta modalTarget='modal-wormhole-testing' betaNav='beta-wormhole-testing'}}`);
  ...
  test('it can close the about dialog', async function(assert) {
    let beta = generateBetaTrial();
    beta.isBetaSite = true;

    this.set('beta', beta);
    await render(
      hbs`{{beta-trial beta=beta showModal=true modalTarget='modal-wormhole-testing' betaNav='beta-wormhole-testing'}}`
    );
  ...
  test('it can close the about dialoag by clicking outside the modal', async function(assert) {
    let beta = generateBetaTrial();
    beta.isBetaSite = true;
  ...
  test('it can exit the beta', async function(assert) {
    assert.expect(2);
    let beta = generateBetaTrial();
    beta.isBetaSite = true;
    this.set('externalAction', () => {
    ...
    await render(hbs`{{beta-trial betaNav='beta-wormhole-testing' beta=beta exitBeta=(action externalAction)}}`);
  ...
  test('it renders an exit interview', async function(assert) {
    const withExit = true;
    let beta = generateBetaTrial(withExit);
    this.set('beta', beta);

    await render(hbs`{{beta-trial beta=beta modalTarget='ember-testing'}}`);
