...
export default function() {
  ...
  this.get(`${baseUrl}/api/v1/list/comments/:typeId/:storyId/`, () => ({ results: server.createList('comment', 5) }) );

  this.get(`${baseUrl}/api/v2/related/:storyId/`, function(schema) {
    server.createList('story', 5);
    return {
      results: schema.stories.all().models.map(c => c.attrs)
    };
  });
  ...
  this.get('/story/:slug', function(schema, {params: {slug}}) {
    ...
    if (!djangoPage) {
      ...
      djangoPage = server.create('djangoPage', {
        id: id,
        slug: slug
      });
    }
  ...
  this.get(`${baseUrl}`, function(schema) {
    ...
    if (!home) {
      home = server.create('djangoPage', {id:'/', slug:'story/'});
    }
