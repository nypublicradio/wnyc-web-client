export default function(code) {
  ...
  const fields = ['slug', 'audio', 'audioduration', 'video', 'modelchar', 'domainint', 'hasshow', 'haschannel', 'isnpr', 'channeltype', 'seriestitles', 'channeltitle', 'showtitle', 'tags', 'audiopath'];
  const REGEX = /(?:.*:)?(.*)\W+\$A(\d)\$AD(\d+)\$V(\d)\$M(\w)\$D(\d+)\$HS(\d)\$HC(\d)\$N(\d)\$CT(\w)\$SS([^$]+)\$C([^$]*)\$S([^$]*)\$T([^$]+)\$AP([^$]+)\$$/;
  ...
  const boolFields = ['audio', 'video', 'haschannel', 'hasshow', 'isnpr'];
